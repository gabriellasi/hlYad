...
foreach($service as $serviceName){
 ...
 $checkService ="SELECT * from youth_shelter WHERE service='$serviceName' AND email_id='$userMail'";
 $resCheck = $conn->query($checkService);
 
 if($resCheck->num_rows==0){
     $insertUserChoice = "INSERT into youth_shelter (email_id,service) VALUES ('".$userMail."','".$serviceName."')";
...
if($resultCheck -> num_rows > 0){
    $data['success']=true;
    $data['message']= "הודעתך הועברה לגופים המטפלים,".'<br>'."חשוב לנו לדעת שקיבלת מענה הולם ונשמח שתעדכן/י באיזור האישי את חוות דעתך ".'&#9825'.'<br>'.'<a href="http://http://gabriellasi.mtacloud.co.il/PersonalErea/personalPage.php" target="_blank">האיזור האישי שלך</a>';
}
else{
    $data['success']=false;
