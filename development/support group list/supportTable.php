...
<html lang="he" dir="rtl">
    <head>
        ...
        <link rel="stylesheet" type="text/css" href="supportTable.css">
    ...
    <body>
        <div class="container">
            ...
            <div class="text-center">
             ...
             <a href="http://gabriellasi.mtacloud.co.il/support%20group%20list/supportTable.php" onclick="signOut();" id="signOut" style="display:none;"> התנתק</a>
            <script>
                function signOut() {
                    gapi.auth2.init();
              ...
              <div class="row">
                <div class="col-lg-6 col-lg-offset-3 text-center">
                    <a href="#" id="find_a_place">
                        <p>לחץ/י למציאת קבוצת תמיכה בהתאמה אישית
                        </p>
                    </a>
                </div>
              </div>
...
$supportGroupSQL = "SELECT support_group.name, risk_type.type as type, support_group.phone, support_group.address, support_group.mail, support_group.website_url FROM support_group JOIN risk_type ON support_group.risk=risk_type.ID" ;
$supportGroupRes=$conn->query($supportGroupSQL);
...
echo "<h2 id='$Title'>רשימת קבוצות תמיכה</h2>";
echo "<div class='$ClassName'>";
echo "<table class='$Table' >
    <tr><th> שם קבוצת תמיכה </th><th>תחום סיכון</th><th>טלפון</th><th>כתובת</th><th>מייל</th><th>אתר אינטרנט</th></tr>";

    if($supportGroupRes->num_rows > 0){
    while($rowGroup = $supportGroupRes->fetch_assoc()){
        echo "<tr><td>".$rowGroup["name"]."</td><td>".$rowGroup["type"]."</td><td>".$rowGroup["phone"]."</td><td>".$rowGroup["address"]."</td><td>".$rowGroup["mail"]."</td><td>".$rowGroup["website_url"]."</td></tr>";  
    }
}
