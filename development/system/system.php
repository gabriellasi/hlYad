...
//we want to echo the first shelter by using address from $percentShelterCombine

$addressForSelect = key($percentShelterCombine);
echo "this is the best address"."<br>";
echo $addressForSelect;

$resultShelters = "SELECT name FROM shelter WHERE address ='$addressForSelect'";
$res = $conn->query($resultShelters);

if($res->num_rows > 0){
    while($row = $res->fetch_assoc()){
        echo $row["name"];
        echo $percentShelterCombine[$addressForSelect];
    }
}
else{
    echo "0 results";
